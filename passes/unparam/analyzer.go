...
var Analyzer = &analysis.Analyzer{
	Name:     "unparam",
	Doc:      "check for unused parameters and results in functions",
	Run:      run,
	Requires: []*analysis.Analyzer{buildssa.Analyzer},
}

var tests, exported, debug bool

func init() {
	Analyzer.Flags.BoolVar(&tests, "tests", false, "load tests too")
	Analyzer.Flags.BoolVar(&exported, "exported", false, "inspect exported functions")
	//Analyzer.Flags.BoolVar(&debug, "debug", false, "debug prints")
}
...
type checker struct {
	...
	cachedDeclCounts map[string]int
...
func (c *checker) run() (interface{}, error) {
	...
	for _, f := range c.pass.Files {
		if len(f.Comments) > 0 && generatedDoc(f.Comments[0].Text()) {
			fname := c.pass.Fset.Position(f.Pos()).Filename
			genFiles[fname] = true
		}
		ast.Inspect(f, func(node ast.Node) bool {
			...
			case *ast.ValueSpec:
				...
				iface, ok := c.pass.TypesInfo.TypeOf(node.Type).Underlying().(*types.Interface)
				if !ok {
					break
				}
				// var _ someIface = named
				valTyp := c.pass.TypesInfo.Types[node.Values[0]].Type
	...
	for _, curFunc := range c.ssainput.SrcFuncs {
	...
	for _, fn := range c.ssainput.SrcFuncs {
		switch {
		...
		fname := c.pass.Fset.Position(fn.Pos()).Filename
		if !tests && strings.HasSuffix(fname, "_test.go") {
			continue
		}
		if exported || fn.Pkg.Pkg.Name() == "main" {
...
func (c *checker) addIssue(fn *ssa.Function, pos token.Pos, format string, args ...interface{}) {
	...
	c.pass.Reportf(pos, fname+" - "+format, args...)
}
...
resLoop:
	...
	for i, par := range fn.Params {
		...
		if c.pass.TypesSizes.Sizeof(par.Type()) == 0 {
...
func (c *checker) anyRealUse(par *ssa.Parameter, pos int) bool {
	refs := *par.Referrers()
	if len(refs) == 0 {
		...
		ast.Inspect(body, func(node ast.Node) bool {
			...
			if right, ok := asgn.Rhs[0].(*ast.Ident); ok {
				obj := c.pass.TypesInfo.Uses[right]
...
func (c *checker) declCounts(pkgDir string, pkgName string) map[string]int {
	if c.cachedDeclCounts != nil {
		return c.cachedDeclCounts
	}
	fset := token.NewFileSet()
	...
	pkg := pkgs[pkgName]
	c.cachedDeclCounts = make(map[string]int)
...
func (c *checker) multipleImpls(fn *ssa.Function) bool {
	if fn.Parent() != nil { // nested func
		return false
	}
	path := c.pass.Fset.Position(fn.Pos()).Filename
	count := c.declCounts(filepath.Dir(path), c.pass.Pkg.Name())
	name := fn.Name()
	if recv := fn.Signature.Recv(); recv != nil {
		named := findNamed(recv.Type())
